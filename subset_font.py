...
# ✅ 5. WOFF2 압축 실행 (Python 내부에서 `woff2_compress` 사용)
try:
    subprocess.run(["woff2_compress", output_font_ttf], check=True)
    print(f"✅ 최적화된 WOFF2 폰트 생성 완료: {output_font_woff2}")
except FileNotFoundError:
    print("❌ `woff2_compress` 실행 파일을 찾을 수 없습니다. 시스템에 설치했는지 확인하세요.")
except subprocess.CalledProcessError as e:
    print(f"❌ WOFF2 압축 과정에서 오류 발생: {e}")

# # ✅ 5. WOFF2 포맷 저장 + 브로틀리 압축 (최적화된 압축)
# woff2_data = font.saveXML(output_font_woff2)
# compressed_data = brotli.compress(woff2_data, mode=brotli.MODE_TEXT, quality=11)
#
# with open(output_font_woff2, "wb") as f:
#     f.write(compressed_data)
#
# print(f"✅ 최적화된 서브셋 폰트 생성 완료: {output_font_woff2} (압축 적용)")
